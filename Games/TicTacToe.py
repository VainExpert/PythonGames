...
class TicTacToe(GameUtils):
    ...
    def __init__(self, width=660, height=660, cols=3, rows=3, player=None):
    ...
    def all_possible_moves(cls, board):
        ...
        for i in range(len(board)):
            for j in range(len(board[i])):
                if board[i][j] == 0:
                    possible_moves.append(TicTacToeMove(i, j, idx))
                    idx += 1
    ...
    def solved_moves(self):
        ...
        for corner in corners:
            first_turns.append(TicTacToeMove(corner[0], corner[1]))

        defences = []
        for react in reactions:
            defences.append(TicTacToeMove(react[0], react[1]))
    ...
    def rescale_img(self):
        if self.width != 660 and not self.rescaled:
            if self.player == 0:
                self.player_img = pygame.transform.scale(pygame.image.load("C:/Users/Tom/Documents/Coding/Code/GitHub/PythonGames/Games/X_SYM.png"), (self.IMG_SIZE, self.IMG_SIZE))
